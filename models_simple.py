...
class SimplIE(torch.nn.Module):
    ...
    def forward(self, s, r, o, flag_debug=0):
        ...
        if o is None or o.shape[1] > 1: 
            ...
            if o is not None:
                ...
                o_e_h = o_e_h.view(-1,self.embedding_dim).transpose(0,1)
                result = tmp1 @ o_e_h
    ...
    def regularizer(self, s, r, o):
        ...
        o = self.E_o(o)
        if self.reg==2:
            return (s*s+o*o+r*r).sum()/2
        elif self.reg==22:
            return ((torch.norm(self.E_s.weight, p=2) ** 2) + (torch.norm(self.E_o.weight, p=2) ** 2) + (torch.norm(self.R.weight, p=2) ** 2) + (torch.norm(self.R_inv.weight,p=2)**2) ) / 2 
